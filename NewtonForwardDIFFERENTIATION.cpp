...
double secondDef(vector<vector<double>> &y,vector<double> &x,double xth,int n,double h)
{
    vector<double> constant = {1,-1,.9166,-.8333};
    int si = constant.size();
    int i =0;
    for(;i<n;i++)
    {
        if(xth==x[i]) break;
    ...
    double sum =0;
    for(int j =2,k =0;k<si;j++,k++)
    {
        cout<<"DEBUG-> "<<sum<<" "<<y[i][j]<<" "<<constant[k]<<endl;
        sum+= (y[i][j] * constant[k] );
    }
    h *=h;
    sum /= h;
    return sum;
...
int main()
{
    ...
    */
    ...
    cout<<secondDef(y,x,x[1],n,x[1]-x[0])<<endl;
    return 0;
}
